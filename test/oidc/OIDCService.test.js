...
import {mockConfigurationService} from "../mocks/ConfigurationService.mock.js";
import {mockStorageService} from "../mocks/StorageService.mock.js";
import {mockExpirationService} from "../mocks/ExpirationService.mock.js";

const {ConfigurationService} = await import("../../src/oidc/ConfigurationService.js");
const {StorageService} = await import("../../src/oidc/StorageService.js");
const {ExpirationService} = await import("../../src/oidc/ExpirationService.js");
...
describe("ConfigurationService constructor", function() {

  it("nominal",  function() {
    //WHEN
    const oidcService = new OIDCService(authorityUrl, clientId)

    expect(oidcService.clientId).toEqual(clientId);
    expect(oidcService.configurationService).toEqual(mockConfigurationService);

    //THEN
    expect(ConfigurationService).toHaveBeenCalledTimes(1)
    expect(StorageService).toHaveBeenCalledTimes(1)
    expect(ExpirationService).toHaveBeenCalledTimes(1)
    expect(ExpirationService).toHaveBeenCalledWith(oidcService.WATCHER_ACTIONS)
    expect(mockExpirationService.watchExpiration).toHaveBeenCalledTimes(1)
  });
});
